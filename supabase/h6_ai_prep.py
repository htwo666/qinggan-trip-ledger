#!/usr/bin/env python3
# h6_ai_prep.py — 更新 AI prep 类型和 prompt
import sys
src = open('index.html', encoding='utf-8').read()

# 1) 重写 confirmAIPrep
old_cap = r"""function confirmAIPrep(o){
...
NEW_CAP = r"""function confirmAIPrep(o){
...
# 3) 更新 prompt 里 prep 的指令说明
old_desc = "  - prep：name 必填；category 从 服饰装备/个护美妆/药品保健/证件/食品/其他 里挑；price/channel 可选\n"
new_desc = "  - prep：name 必填；category 从 服饰装备/个护美妆/药品保健/证件/食品/其他 里挑，或自定义板块名\n"
if src.count(old_desc) != 1:
    print(f'!! prompt prep desc ({src.count(old_desc)})')
    sys.exit(1)
